...
# Считаем активную мощность узлов
activePower = []
count_dots = int(input("Введите колличество узлов: "))
for i in range(count_dots - 1):
    active = int(input("Введите активную мощность узла: "))
    activePower.append(active)
print("Активные мощности узлов:", activePower)

# Считаем сумму мощностей генераторов
totalAmount = 0
for i in range(len(activePower)):
    total = activePower[i]
    totalAmount += total
sumPower = 0.95 * totalAmount + 0.06 * totalAmount
sumPower = float("{0:.1f}".format(sumPower))
print("Сумма активной мощности генераторов: ", sumPower)

# Считаем реактивную мощность генераторов
tgA = float(input("Введите коэффициент мощности: "))
reactivePower = sumPower * tgA
reactivePower = float("{0:.1f}".format(reactivePower))
print("Реактивная мощность генераторов: ", reactivePower)
print("----------------------------------")

with open('resultData.txt', 'w') as fileResult:
    fileResult.write("Активные мощности узлов:\n" + str(activePower))
    fileResult.write("\nСумма активной мощности генераторов: " + str(sumPower))
    fileResult.write("\nРеактивная мощность генераторов: " + str(reactivePower))
